...
class PixelizedSource(object):
    def __init__(self, masked_imaging, pixelization_shape_2d=(30,30)):
    ...
    def build_reg_matrix(self, lam_s=0.1):
        self.lam_s = lam_s
    ...
    def source_inversion(self, pixelized_mass_obj, lam_s=0.1, new_lens_mapping=True):
        if new_lens_mapping:
            self.build_lens_mapping(pixelized_mass_obj)

        self.build_reg_matrix(lam_s)
    ...
    def find_best_regularization(self, pixelized_mass_obj, log10_lam_range=[-5, 4]):
        '''
        ...
        self.build_lens_mapping(pixelized_mass_obj)
        t0 = time.time()
        self.best_fit_reg_info = dual_annealing(self.regularization_merit, bounds=[log10_lam_range,])
        t1 = time.time()
