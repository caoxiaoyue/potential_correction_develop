...
class IterativePotentialCorrect(object):
    ...
    def initialize_iteration(
        ...
        self.lam_dpsi_type = lam_dpsi_type
    ...
    def RTR_matrix_from(self, pix_src_obj, lam_s, lam_dpsi, scale_dpsi):
        ...
        if self.lam_dpsi_type == '4th':
            self.grid_obj.get_diff_4th_reg_operator_dpsi()
            HTH_dpsi = np.matmul(self.grid_obj.Hx_dpsi_4th_reg.T, self.grid_obj.Hx_dpsi_4th_reg) + \
                np.matmul(self.grid_obj.Hy_dpsi_4th_reg.T, self.grid_obj.Hy_dpsi_4th_reg)
        elif self.lam_dpsi_type == '2nd':
            self.grid_obj.get_diff_2nd_reg_operator_dpsi()
            HTH_dpsi = np.matmul(self.grid_obj.Hx_dpsi_2nd_reg.T, self.grid_obj.Hx_dpsi_2nd_reg) + \
                np.matmul(self.grid_obj.Hy_dpsi_2nd_reg.T, self.grid_obj.Hy_dpsi_2nd_reg)
        elif self.lam_dpsi_type == 'gauss':
            self.grid_obj.get_gauss_reg_operator_dpsi(scale=scale_dpsi)
            HTH_dpsi = self.grid_obj.gauss_reg_dpsi
        elif self.lam_dpsi_type == 'exp':
            self.grid_obj.get_exp_reg_operator_dpsi(scale=scale_dpsi)
            HTH_dpsi = self.grid_obj.exp_reg_dpsi   

        RTR_matrix[self._ns:, self._ns:] = np.copy(lam_dpsi * HTH_dpsi) #Note, not lam_dpsi**2
    ...
    def Mc_RTR_matrices_from(
        ...
        self.RTR_matrix = self.RTR_matrix_from(self.pix_src_obj, lam_s, lam_dpsi, scale_dpsi)
    ...
    def run_this_iteration(self):
        self.run_sampling()
        self.evidence_this_iter(self.quantiles[0,1], self.quantiles[1,1], self.quantiles[2,1]) #???? TODO
    ...
    def loglike(self, params):
        ...
        lam_dpsi=params[1]
        scale_dpsi=params[2]
        return self.evidence_this_iter(lam_s, lam_dpsi, scale_dpsi)


    def prior_tranfer(self, unit_cube):
        unit_cube[0] = 10**( (np.log10(1e5)-np.log10(1e-5))*unit_cube[0] + np.log10(1e-5) )
        unit_cube[1] = 10**( (np.log10(1e15)-np.log10(1e5))*unit_cube[1] + np.log10(1e5) )
        unit_cube[2] = (10.0-0.0001)*unit_cube[2] + 0.0001
    ...
    def run_sampling(
        ...
        self.sampler = dynesty.NestedSampler(
            self.loglike, self.prior_tranfer, 3, nlive=nlive, bound=bound, sample=sample,
        ...
        self.weights = self.results.importance_weights()
        self.quantiles = [dyfunc.quantile(samps, [0.5-0.68/2.0,0.5,0.5+0.68/2.0], weights=self.weights) for samps in self.samples.T]
